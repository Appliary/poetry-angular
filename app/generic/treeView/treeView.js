app.directive( 'treeView', function ( $q, $timeout, ViewDataSource, ngDialog, ngNotify, $filter ) {
    ...
    return {
        ...
        controller: function ( $scope ) {
            $scope.controlId = 'tree_view_' + Math.random()
                .toString( 36 )
                .replace( /[^a-z]+/g, '' )
                .substr( 0, 10 );
            ...
            $scope.tvActions = {
                initTreeControl: function () {
                    ...
                    $jsTree.bind( 'select_node.jstree', function ( node, selected, event ) {
                        ...
                        if ( boData.isRoot && ( $scope.isGroups || !$scope.boMeta[ boType ].editTemplate ) ) {
                            ...
                            if ( $scope.boMeta[ boType ].edit ) {
                                if ( !boData.isRoot ) {
                                    boData.parent = $.jstree.reference( this )
                                        .get_json( selected.node.parent )
                                        .data;
                                }
                    ...
                    $jsTree.bind( 'move_node.jstree', function ( event, params ) {
                        ...
                        var newParent = $.jstree.reference( _controlSelector )
                            .get_node( params.parent );
                        if ( newParent.data && newParent.data.boType === params.node.data.boType ) {
                            ...
                            if ( parentMeta.canHave && parentMeta.canHave.indexOf( newParent.data.boType ) === -1 ) {
                                var moveParent = $.jstree.reference( _controlSelector )
                                    .get_node( newParent.parent );
                    ...
                    if ( $scope.isGroups ) {
                        $jsTree.bind( 'delete_node.jstree', function ( event, node ) {
                            var parent = $.jstree.reference( this )
                                .get_json( node.parent )
                                .data;
            ...
            var initContextMenu = function ( node ) {
                var result = {};
                var obj = $.jstree.reference( _controlSelector )
                    .get_json( node ),
                    parent = $.jstree.reference( _controlSelector )
                ...
                if ( contextItems ) {
                    for ( var i = 0, len = contextItems.length; i < len; i++ ) {
                        ...
                        if ( typeof item.action === "string" ) {
                            ...
                            if ( actions[ 0 ] === 'add' ) {
                                action.action = _getAddItemActon( actions[ 1 ], obj.data )
                                    .action;
                            }
            ...
            var _getAssetDeleteAction = function ( obj, parent ) {
                return {
                    ...
                    action: function ( event ) {
                        _openDeleteConfim( $scope.boMeta[ obj.data.boType ] )
                            .then( function () {
                    ...
                    action: function ( event ) {
                        if ( _meta.onAdd ) {
                            ...
                            if ( $scope.isGroups ) {
                                ...
                                newItem.data[ prop ] = parent.isRoot ? {
                                    _id: "root",
                                    name: $filter( 'translate' )( "Root:EnergyBlocks:Column" )
                                } : parent.data;
                            }
